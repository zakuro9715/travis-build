...
describe Travis::Build::Script::Php, :sexp do
  ...
  describe 'installs php nightly' do
    ...
    xit { should include_sexp [:cmd, 'curl -s -o archive.tar.bz2 https://s3.amazonaws.com/travis-php-archives/php-nightly-archive.tar.bz2 && sudo tar xjf archive.tar.bz2 --directory /', timing: true, echo: true] }
  end
  ...
  describe 'when desired PHP version is not found' do
    ...
    xit 'installs PHP version on demand' do
      expect(sexp).to include_sexp [:cmd, "curl -s -o archive.tar.bz2 https://s3.amazonaws.com/travis-php-archives/php-#{version}-archive.tar.bz2 && sudo tar xjf archive.tar.bz2 --directory /", timing: true, echo: true]
    end
  end
